...
Rcpp::List Get_PValue_Lambda(arma::vec &lambda, arma::vec &Q, arma::ivec &df1){
  ...
  for (int i = 0; i < n1; i++) {
    ...
    if (lambda.n_elem == 1) {
      p_val[i] = p_val_liu[i];
    } else if (Rcpp::as<int>(out["ifault"]) != 0) {
      is_converge[i] = 0;
    }
...
// Module-local counter so users can grep stderr for [FELIX] lines and
...
std::string get_jointScore_pvalue(arma::vec& Score, arma::mat& Phi) {
    ...
    if (!ok) {
        ...
        Rcpp::Rcerr << "[FELIX] get_jointScore_pvalue: Phi singular at"
        ...
        if (!ok_pinv) {
            Rcpp::Rcerr << "[FELIX] get_jointScore_pvalue:"
