...
// Convert a two-sided p-value to the corresponding |Z| (upper-tail normal quantile),
// working entirely in log space to avoid two failure modes:
//   (1) forming p/2 on the probability scale, which underflows to 0 for extreme p and
//       makes boost::math::quantile throw a gamma_q_inv overflow, and
//   (2) the incorrect log-scale transform log(p)/2; the correct upper-tail log-probability
//       for a two-sided test is log(p/2) = log(p) - log(2).
// is_log_p indicates whether p is already stored as its natural log.
static inline double two_sided_p_to_abs_z(double p, bool is_log_p) {
    if (std::isnan(p)) return std::numeric_limits<double>::quiet_NaN();
    const double log_half_p = is_log_p
        ? p - M_LN2
        : (p > 0.0 ? std::log(p) - M_LN2
                   : -std::numeric_limits<double>::infinity());
    return std::fabs(R::qnorm(log_half_p, 0.0, 1.0, /*lower_tail=*/false, /*log_p=*/true));
}
...
/*		std::cout << "NAsigma is " << NAsigma << std::endl;
  ...
  */
    ...
    if(t_isSPAConverge){
        try {
          t_qval = two_sided_p_to_abs_z(t_SPApval, ispvallog);
   ...
   if(m_traitType!="quantitative"){
	...
	if(!ispvallog){
		if(m_is_Firth_beta && pval <= m_pCutoffforFirth){
			t_isFirth = true;
			t_qval_Firth = two_sided_p_to_abs_z(pval, false);
		}

	}else{
		if(m_is_Firth_beta && pval <= std::log(m_pCutoffforFirth)){
			t_isFirth = true;
			t_qval_Firth = two_sided_p_to_abs_z(pval, true);
		}
	}
   ...
   double __tractor_saige_condition_start = tractor_hybrid_saige_now_seconds();
	...
	arma::vec t_varT_ctemp = t_G1tilde_P_G2tilde * m_VarInvMat_cond * (t_G1tilde_P_G2tilde.t());
	 ...
	 if(m_traitType!="quantitative" && t_isSPAConverge){
	     ...
	     // log-space two-sided p to |Z| (avoids p/2 underflow crash and the log(p)/2 error)
	     t_qval_update = two_sided_p_to_abs_z(pval, ispvallog);
