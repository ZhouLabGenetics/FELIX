...
if(t_regionTestType == "BURDEN"){
 ...
 //printTime(timeoutput2, timeoutput3, "burden test done");
  ...
  //std::cout << "okk5" << std::endl;
      ...
      VarMatAdjCond = AdjCondMat * (G1tilde_P_G2tilde_Weighted_Mat.t());
      TstatAdjCond = AdjCondMat * (ptr_gSAIGEobj->m_Tstat_cond % w0G2Vec_cond ); 
 ...
 /*}else{ //if(!g_isadmixed){
  ...
  if(numofAnc > 0){
   ...
   if (t_traitType == "binary") {
        ...
        get_newPhi_scaleFactor_cpp(q_sum,
                                mu_a,
                                g_sum,
                                p_new,
                                Scorevec,
	...
	if(t_traitType == "binary"){	
		...
		G1tilde_P_G2tilde_Mat_scaledTemp.each_row() %= arma::sqrt(ptr_gSAIGEobj->m_scalefactor_G2_cond).t();
		arma::mat adjCondTemp = G1tilde_P_G2tilde_Mat_scaledTemp * (ptr_gSAIGEobj->m_VarInvMat_cond_scaled_weighted);
		arma::mat VarMatAdjCondTemp = adjCondTemp * (G1tilde_P_G2tilde_Mat_scaledTemp.t());
		arma::vec TstatAdjCondTemp = adjCondTemp * (ptr_gSAIGEobj->m_Tstat_cond);
		Phi_cond = VarMat_sub; 
...
void mainAdmixedInCPP_inner(
  ...
  unsigned int markers_per_chunk_in_groupTest = 10;
  arma::mat P1Mat = arma::zeros<arma::mat>(markers_per_chunk_in_groupTest, t_n);
  arma::mat P2Mat = arma::zeros<arma::mat>(t_n, markers_per_chunk_in_groupTest);

  unsigned int q_anno_maf = 1;
  arma::mat w0G2Mat_cond(q_cond, q_cond);
  w0G2Mat_cond = w0G2Vec_cond * (w0G2Vec_cond.t());
  arma::mat genoSumMat(t_n, q_anno_maf, arma::fill::zeros); //for Phi_cc for binary traits and BURDEN test
  arma::vec genoSumcount_noweight(q_anno_maf, arma::fill::zeros);
...
//arma::vec timeoutput3a = getTime();
   ...
   if((missingRate > g_missingRate_cutoff) || (MAF > g_maxMAFLimit) || (MAF < g_marker_minMAF_cutoff) || (MAC < g_marker_minMAC_cutoff) || (imputeInfo < g_marker_minINFO_cutoff)){
    ...
    //nNonZero = indexNonZeroVec_arma.n_elem;
			   ...
			   double t_pvalcutoff_of_haplotype)
{
  ...
  arma::mat G1tilde_P_G2tilde_Mat(t_NumberofANC, t_NumberofANC);
 ...
 int numancresults = 0;
  ...
  for(int i = 0; i < q; i++){
    ...
    arma::vec nanc_case_vec(t_NumberofANC);
...
*/
  ...
  for(int j = 0; j < (t_NumberofANC+1); j++){
   ...
   //std::cout << "n " << n << std::endl;
	...
	if(j ==  t_NumberofANC){
		nanc_case = 2*(ptr_gSAIGEobj->m_case_indices).n_elem;
		nanc_ctrl = 2*(ptr_gSAIGEobj->m_ctrl_indices).n_elem;         
	}else{
		nanc_case = nanc_case_vec(j);
		nanc_ctrl = nanc_ctrl_vec(j);	 
	}
...
//printTime(timeoutput5, timeoutput6, "Unified_getMarkerPval");
   ...
   indexZeroVec_arma.clear();
	...
	if(numancresults < t_NumberofANC){
		...
		Scorevecsub = Scorevec.elem(not_nan_indices);
	...
	arma::mat G1tilde_P_G2tilde_Matsub;
  	if(numancresults < t_NumberofANC){
                arma::uvec not_nan_indices = arma::find_finite(Scorevec);
		G1tilde_P_G2tilde_Matsub = G1tilde_P_G2tilde_Mat.rows(not_nan_indices);
		
        }else{
		G1tilde_P_G2tilde_Matsub = G1tilde_P_G2tilde_Mat;
        }

	    
      	    arma::mat AdjCondMat = G1tilde_P_G2tilde_Matsub * (ptr_gSAIGEobj->m_VarInvMat_cond);
            arma::mat VarMatAdjCond = AdjCondMat * (G1tilde_P_G2tilde_Matsub.t());
...
bool openOutfile_single_admixed_new(std::string t_traitType, bool t_isImputation, bool isappend, bool t_isMoreOutput, int t_NumberofANC){
 bool isopen;
	...
	std::string ancstr;
        OutFile_single << "CHR\tPOS\tMarkerID\tAllele1\tAllele2\t";
        for(unsigned int k = 0; k < (t_NumberofANC+1); k++){                
	
             if(k != t_NumberofANC){
                ancstr = "_anc"+std::to_string(k+1);
             }else{
                ancstr = "_ancALL";
             }
             if(k > 0){
             ...
             OutFile_single << "AC_Allele2";
             OutFile_single << ancstr;;
             OutFile_single << "\t";
             OutFile_single << "AF_Allele2";
             OutFile_single << ancstr;;
             if(t_isImputation){
			OutFile_single << "\t";
                        OutFile_single << "imputationInfo";
			...
			OutFile_single << ancstr;
             ...
             OutFile_single << ancstr;;
             OutFile_single << "\t";
             OutFile_single << "Tstat";
             OutFile_single << ancstr;;
             OutFile_single << "\t";
             OutFile_single << "var";
             OutFile_single << ancstr;;
             OutFile_single << "\t";
             OutFile_single << "p.value";
             OutFile_single << ancstr;;
             if(t_traitType == "binary" || t_traitType == "survival"){
             	OutFile_single << "\t";
                OutFile_single << "p.value.NA";
                OutFile_single << ancstr;;
                OutFile_single << "\t";
             ...
             //if(ptr_gSAIGEobj->m_isCondition){
                ...
                OutFile_single << ancstr;;
                OutFile_single << "\t";
                OutFile_single << "Tstat_c";
                OutFile_single << ancstr;;
                OutFile_single << "\t";
                OutFile_single << "var_c";
                OutFile_single << ancstr;;
                OutFile_single << "\t";
                OutFile_single << "p.value_c";
                OutFile_single << ancstr;;
                if(t_traitType == "binary" || t_traitType == "survival"){
             ...
             if(t_traitType == "binary" || t_traitType == "survival"){
                OutFile_single << "\t";
                OutFile_single << "AF_case";
                OutFile_single << ancstr;
                OutFile_single << "\t";
                OutFile_single << "AF_ctrl";
                OutFile_single << ancstr;
                OutFile_single << "\t";
                OutFile_single << "N_case";
                OutFile_single << ancstr;;
                OutFile_single << "\t";
                OutFile_single << "N_ctrl";
                OutFile_single << ancstr;;

                if(t_isMoreOutput){
                OutFile_single << "\t";
                OutFile_single << "N_case_hom";
                OutFile_single << ancstr;
                OutFile_single << "\t";
                OutFile_single << "N_case_het";
                OutFile_single << ancstr;
                OutFile_single << "\t";
                OutFile_single << "N_ctrl_hom";
                OutFile_single << ancstr;;
                OutFile_single << "\t";
                OutFile_single << "N_ctrl_het";
                OutFile_single << ancstr;;
                //OutFile_single << "\t";
                }
          }else if(t_traitType == "quantitative"){//if(t_traitType == "binary" || t_traitType == "survival")
...
/*
                OutFile_single << "\t";
                OutFile_single << "Pvalue_SKATO";
                OutFile_single << "\t";
                OutFile_single << "Pvalue_Burden";
*/
/*
                OutFile_single << "\t";
                OutFile_single << "BETA_Burden";
                OutFile_single << "\t";
                OutFile_single << "SE_Burden";
 */
                OutFile_single << "\t";
...
/*
                ...
                OutFile_single << "\t";
                OutFile_single << "Pvalue_Burden_c";
*/
/*
                OutFile_single << "\t";
                OutFile_single << "BETA_Burden_c";
                OutFile_single << "\t";
                OutFile_single << "SE_Burden_c";
*/
                OutFile_single << "\t";
                OutFile_single << "P_het_admixed_c";
                OutFile_single << "\t";
                OutFile_single << "P_hom_admixed_c";
                OutFile_single << "\t";
                           ...
                           )           // sample size
{
  ...
  for(unsigned int i = 0; i < t_NumberofANC; i++)
  {
    // marker-level information
    double altFreq, altCounts, missingRate, imputeInfo;
    std::vector<uint32_t> indexForMissing;
    ...
    bool isOutputIndexForMissing = true;
    bool isOnlyOutputNonZero = false;
    std::string vcfFieldtoRead = "ANC" + std::to_string(i+1);
  ...
  if(t_traitType == "binary" || t_traitType == "survival"){
        ...
        nanc_case_vec(i) = nanc_case;
        nanc_ctrl_vec(i) = nanc_ctrl;
   }
