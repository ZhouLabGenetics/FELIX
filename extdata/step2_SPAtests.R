...
option_list <- list(
  ...
  make_option("--FELIXlaPrefix", type="character",default="",
    help="Prefix for the FELIXla packed step2 input (produced by `felixla --make-felixla`)."),
  make_option("--tractorHybridPrefix", type="character",default="",
    help="Deprecated alias for --FELIXlaPrefix; kept for backward compatibility."),
...
mean, p-value based on traditional score test is returned. Default value is 2."),
 ...
 make_option("--is_admixed", type="logical", default=FALSE,
...
## --FELIXlaPrefix is the current flag; --tractorHybridPrefix is kept as a backward-compatible alias.
felixlaPrefix <- if (opt$FELIXlaPrefix != "") opt$FELIXlaPrefix else opt$tractorHybridPrefix
...
if(nThreads == 1){
  SPAGMMATtest(vcfFile=opt$vcfFile,
             ...
             tractorHybridPrefix=felixlaPrefix,
